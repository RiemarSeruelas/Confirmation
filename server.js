...
const app = express();
const PORT = Number(cleanEnv(process.env.PORT) || 5178);
...
function secretMatches(value, expected) {
  const providedBuffer = Buffer.from(String(value || ""), "utf8");
...
function normalizedClientIp(req) {
  return usageText(req.ip || req.socket?.remoteAddress || "unknown", 80)
    .replace(/^::ffff:/, "");
}
...
async function upsertUsageSession(entry) {
  if (!USAGE_LOG_ENABLED) return;

  await pool.query(
    `
      INSERT INTO app."Confirmation-logs" AS existing_session (
        event_type,
        session_id,
        operator_id,
        operator_name,
      ...
      VALUES ('session', $1, $2, $3, $4, $5, $6, $7, $8, $9, NOW())
      ON CONFLICT (session_id) DO UPDATE
      SET
        event_type = 'session',
        last_seen_at = NOW(),
        operator_id = COALESCE(EXCLUDED.operator_id, existing_session.operator_id),
        operator_name = CASE
          WHEN EXCLUDED.operator_name <> 'Anonymous' THEN EXCLUDED.operator_name
          ELSE existing_session.operator_name
        END,
        role_name = CASE
          WHEN EXCLUDED.role_name <> 'visitor' THEN EXCLUDED.role_name
          ELSE existing_session.role_name
        END,
        site_name = CASE
          WHEN EXCLUDED.site_name <> 'Unknown' THEN EXCLUDED.site_name
          ELSE existing_session.site_name
        END,
...
async function initializeSchemaWithRetry() {
  try {
    ...
    console.warn(
      `Database is not ready: ${getFriendlyDbError(error)} Retrying in ${Math.round(SCHEMA_RETRY_MS / 1000)} seconds.`
    );
    schemaRetryTimer = setTimeout(initializeSchemaWithRetry, SCHEMA_RETRY_MS);
...
app.post("/api/usage/visit", async (req, res) => {
  try {
    ...
    const sessionId = usageText(req.body?.session_id, 100);
    if (!sessionId) {
      return res.status(400).json({ ok: false, error: "Session ID is required." });
    }

    const userAgent = usageText(req.get("user-agent") || "unknown", 500);

    await upsertUsageSession({
      session_id: sessionId,
      operator_id: Number(req.body?.operator_id || 0) || null,
      operator_name: usageText(req.body?.operator_name, 120) || "Anonymous",
      role_name: usageText(req.body?.role_name, 40) || "visitor",
      site_name: usageText(req.body?.site_name, 80) || "Unknown",
      entry_page: usageText(req.body?.entry_page, 80) || "auth",
      ip_address: normalizedClientIp(req),
    ...
    schemaReadyPromise = null;
    console.error("Database usage log write failed:", error);
    return res.status(500).json({ ok: false, error: "Could not save the usage log to PostgreSQL." });
  }
});
