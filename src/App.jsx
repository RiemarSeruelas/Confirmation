...
function normalizeFields(fields) {
  if (!Array.isArray(fields) || !fields.length) return defaultMachineFields;
...
function normalizeCallouts(callouts) {
  if (!Array.isArray(callouts) || !callouts.length) return defaultCallouts;
  return callouts.map((callout, index) => ({
    ...
    x: Number.isFinite(Number(callout.x)) ? Number(callout.x) : 50,
    y: Number.isFinite(Number(callout.y)) ? Number(callout.y) : 50,
  }));
}
...
function AdminSystemPage() {
  ...
  function resetForm() {
    setForm({ ...emptyMachineForm, fields: defaultMachineFields, callouts: defaultCallouts });
    setSelectedCalloutId(defaultCallouts[0].id);
  }
  ...
  function handlePreviewClick(event) {
    ...
    setForm((current) => ({ ...current, callouts: current.callouts.map((callout) => callout.id === selectedCalloutId ? { ...callout, x: Math.round(x), y: Math.round(y) } : callout) }));
  }
  ...
  async function handleSave(event) {
    ...
    try {
      setSaving(true);
      await fetchJson("/api/admin/machines", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
  ...
  async function handleDelete(machine) {
    ...
    try { await fetchJson(`/api/admin/machines/${machine.id}`, { method: "DELETE" }); resetForm(); await loadMachines(); }
    catch (error) { setMessage(error.message); }
  }

  useEffect(() => { loadMachines().catch((error) => setMessage(error.message)); }, []);
    ...
    <main className="admin-page app-gradient page-pad system-page">
      <section className="system-grid">
        <form className="input-form glass-card machine-builder" onSubmit={handleSave}>
          <div className="form-title-row"><div><p className="eyebrow">Admin System</p><h1>Machine Builder</h1></div><button className="ghost-button" type="button" onClick={resetForm}>New</button></div>
          ...
          <div className="builder-block"><div className="block-head"><h2>Input Fields</h2><button className="secondary-button small" type="button" onClick={() => updateForm("fields", [...form.fields, { id: uid("field"), label: "New Field", type: "text", required: false, mapsTo: "custom" }])}>Add Field</button></div>{form.fields.map((field, index) => <div className="builder-row" key={field.id}><input value={field.label} onChange={(event) => updateField(index, "label", event.target.value)} placeholder="Label" /><select value={field.type} onChange={(event) => updateField(index, "type", event.target.value)}><option value="text">Text</option><option value="number">Number</option><option value="textarea">Paragraph</option></select><select value={field.mapsTo} onChange={(event) => updateField(index, "mapsTo", event.target.value)}><option value="custom">Custom</option><option value="reading_value">Reading</option><option value="product">Product</option><option value="batch_number">Batch</option><option value="remarks">Remarks</option></select><label className="mini-check"><input type="checkbox" checked={field.required} onChange={(event) => updateField(index, "required", event.target.checked)} /> Required</label><button className="ghost-button danger" type="button" onClick={() => updateForm("fields", form.fields.filter((_, i) => i !== index))}>×</button></div>)}</div>

          <div className="builder-block"><div className="block-head"><h2>Callouts</h2><button className="secondary-button small" type="button" onClick={() => { const next = { id: uid("callout"), title: "New Callout", valueKey: "reading_value", x: 50, y: 50 }; updateForm("callouts", [...form.callouts, next]); setSelectedCalloutId(next.id); }}>Add Callout</button></div>{form.callouts.map((callout, index) => <div className={selectedCalloutId === callout.id ? "builder-row selected" : "builder-row"} key={callout.id}><input value={callout.title} onChange={(event) => updateCallout(index, "title", event.target.value)} placeholder="Title" /><select value={callout.valueKey} onChange={(event) => updateCallout(index, "valueKey", event.target.value)}><option value="reading_value">Reading</option><option value="machine_name">Machine</option><option value="site_name">Site</option><option value="operator_name">Operator</option><option value="shift_name">Shift</option><option value="total_submissions">Total</option>{form.fields.map((field) => <option key={field.id} value={field.id}>{field.label}</option>)}</select><button className="secondary-button small" type="button" onClick={() => setSelectedCalloutId(callout.id)}>Locate</button><button className="ghost-button danger" type="button" onClick={() => updateForm("callouts", form.callouts.filter((_, i) => i !== index))}>×</button></div>)}</div>
        ...
        <section className="glass-card builder-preview-card">
          <div className="form-title-row"><div><p className="eyebrow">Preview</p><h2>Locate Callouts</h2></div></div>
          <div className="builder-preview" onClick={handlePreviewClick}>{form.image_data_url ? <img src={form.image_data_url} alt="Machine preview" /> : <div className="machine-visual preview-machine"><div className="vessel" /><div className="motor" /><div className="legs left" /><div className="legs right" /><div className="pipe" /></div>}{form.callouts.map((callout) => <div key={callout.id} className={selectedCalloutId === callout.id ? "preview-pin active" : "preview-pin"} style={{ left: `${callout.x}%`, top: `${callout.y}%` }}>{callout.title}</div>)}</div>
          <div className="machine-list-admin"><p className="eyebrow">Machines</p>{machines.map((machine) => <article key={machine.id} className="machine-admin-row"><button type="button" onClick={() => editMachine(machine)}><strong>{machine.machine_name}</strong><span>{machine.site_name}</span></button><button className="ghost-button danger" type="button" onClick={() => handleDelete(machine)}>Delete</button></article>)}{!machines.length && <p className="empty-state">No machines configured yet.</p>}</div>
        </section>
      </section>
    </main>
  );
...
function MachineViewPage() {
  ...
  async function loadDashboard() { try { const [data, machineData] = await Promise.all([fetchJson("/api/dashboard/summary"), fetchJson("/api/machines")]); setSummary(data.stats || null); setRecords(data.latest || []); setMachines(machineData.machines || []); if (!selectedMachineId && machineData.machines?.[0]) setSelectedMachineId(String(machineData.machines[0].id)); setMessage(data.latest?.length ? "Live database feed" : "No submissions yet"); } catch (error) { setMessage(error.message); } }
  useEffect(() => { loadDashboard(); }, []);
  const selectedMachine = machines.find((machine) => String(machine.id) === String(selectedMachineId)) || machines[0];
  const latest = records.find((record) => selectedMachine?.id && Number(record.machine_config_id) === Number(selectedMachine.id)) || records[0];
  const callouts = normalizeCallouts(selectedMachine?.callouts);
  const isLow = selectedMachine?.threshold_min !== null && selectedMachine?.threshold_min !== undefined && latest?.reading_value !== null && Number(latest?.reading_value) < Number(selectedMachine.threshold_min);
  const isHigh = selectedMachine?.threshold_max !== null && selectedMachine?.threshold_max !== undefined && latest?.reading_value !== null && Number(latest?.reading_value) > Number(selectedMachine.threshold_max);
  const statusText = isLow ? "Below Threshold" : isHigh ? "Above Threshold" : "Live";
  return <main className="machine-page app-gradient page-pad"><section className="machine-monitor scalable-monitor"><aside className="asset-panel"><div className="asset-brand">Unilever</div><dl><div><dt>Asset Name</dt><dd>{selectedMachine?.machine_name || latest?.machine_name || "No Machine"}</dd></div><div><dt>Status</dt><dd className={isLow || isHigh ? "status-warn" : "status-ok"}>{statusText}</dd></div><div><dt>Latest Reading</dt><dd>{formatNumber(latest?.reading_value)}</dd></div><div><dt>Product</dt><dd>{latest?.product || "—"}</dd></div><div><dt>Batch</dt><dd>{latest?.batch_number || "—"}</dd></div><div><dt>Operator</dt><dd>{latest?.operator_name || "—"}</dd></div><div><dt>Last Update</dt><dd>{formatDateTime(latest?.record_timestamp)}</dd></div></dl></aside><section className="process-view"><div className="monitor-head"><div><p className="eyebrow">Machine Interface</p><h1>{selectedMachine?.machine_name || latest?.machine_name || "Machine Monitor"}</h1></div><div className="monitor-actions"><select value={selectedMachineId} onChange={(event) => setSelectedMachineId(event.target.value)}>{machines.map((machine) => <option key={machine.id} value={machine.id}>{machine.machine_name}</option>)}</select><button className="monitor-refresh" type="button" onClick={loadDashboard}>Refresh</button></div></div>{selectedMachine?.details && <p className="machine-view-details">{selectedMachine.details}</p>}<div className="machine-stage dynamic-stage">{selectedMachine?.image_data_url ? <img className="machine-custom-image" src={selectedMachine.image_data_url} alt={selectedMachine.machine_name} /> : <div className="machine-visual" aria-hidden="true"><div className="vessel" /><div className="motor" /><div className="legs left" /><div className="legs right" /><div className="pipe" /></div>}{callouts.map((callout) => <article key={callout.id} className="callout dynamic-callout" style={{ left: `${callout.x}%`, top: `${callout.y}%` }}><span>{callout.title}</span><strong>{valueFromRecord(latest, callout.valueKey, summary)}</strong><small>{message}</small></article>)}</div></section></section></main>;
}
